...
const employees: Employee[] = employeesData.map((empData) => 
  new Employee(empData.id, empData.name, empData.type, cardFactory, externalZone)
);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function populateEmployeeSelect(employees: Employee[]): void {
    const employeeSelect: HTMLSelectElement = document.getElementById('employee-select') as HTMLSelectElement;
  ...
  function updateZoneDisplays(): void {
    // Example for one zone, repeat for others
    const externalZoneElement = document.getElementById('external-zone-employees') as HTMLDivElement;
  ...
  const transactionZoneElement = document.getElementById('transaction-zone-employees') as HTMLDivElement;
  ...
  const operationalZoneElement = document.getElementById('operational-zone-employees') as HTMLDivElement;
  ...
  const secureZoneElement = document.getElementById('secure-zone-employees') as HTMLDivElement;
...
const employeeSelect = document.getElementById('employee-select') as HTMLSelectElement;
...
function getTargetZone(doorId: string): Zone | undefined {
...
function canMoveThroughDoor(employee: Employee, doorId: string): boolean {
    ...
    [transactionZone, [operationalZone, externalZone]]
      , // From 'Transakcji' only to 'Operacyjna'
