...
export class Tester {
  ...
  moveEmployeeToTransactionZone(employeeId: number) {
    const employee = this.cityGallery.employees.find(
      (e) => e.id === employeeId
    );
    const transactionZone = this.cityGallery.zones.find(
      (z) => z.name === "Strefa Transakcji"
    );
    if (employee && transactionZone) {
      this.cityGallery.updateEmployeeZone(employeeId, transactionZone);
      console.log(
        `Pracownik ${employee.fullName} przeniesiony do strefy Transakcji.`
      );
    }
  }

  moveEmployeeToOperationalZone(employeeId: number) {
    const employee = this.cityGallery.employees.find(
      (e) => e.id === employeeId
    );
    const operationalZone = this.cityGallery.zones.find(
      (z) => z.name === "Strefa Operacyjna"
    );
    if (employee && operationalZone) {
      this.cityGallery.updateEmployeeZone(employeeId, operationalZone);
      console.log(
        `Pracownik ${employee.fullName} przeniesiony do strefy Operacyjnej.`
      );
    }
  }

  checkSecurityZoneCapacity() {
    const securityZone = this.cityGallery.zones.find(
      (z) => z.name === "Strefa Zabezpieczeń"
    );
    if (securityZone && securityZone.currentOccupancy > 2) {
      console.error(
        "Błąd: W strefie Zabezpieczeń przebywa więcej niż 2 pracowników."
      );
    } else {
      console.log(
        "W strefie Zabezpieczeń jest odpowiednia liczba pracowników."
      );
    }
  }

  checkJanitorAccess(employeeId: number) {
    const janitor = this.cityGallery.employees.find(
      (e) => e.id === employeeId && e.getCard().type === "Janitor"
    );
    const zone = this.cityGallery.zones.find(
      (z) => z.name === "Strefa Transakcji"
    );
    if (janitor && zone) {
      ...
      if (
        doorToTransaction &&
        doorToTransaction.canEmployeeAccess(
          janitor,
          this.cityGallery.employees,
          zone
        )
      ) {
        ...
        console.error(
          "Dozorca nie może wejść do strefy Transakcji bez innego pracownika."
        );
      }
    }
  }

  checkEmployeeRestrictionInOperationalZone(employeeId: number) {
    const employee = this.cityGallery.employees.find(
      (e) => e.id === employeeId && e.getCard().type === "Trader"
    );
    const operationalZone = this.cityGallery.zones.find(
      (z) => z.name === "Strefa Operacyjna"
    );
    if (
      ...
      console.error(
        "Błąd: Pracownik Transakcji przebywa w Strefie Operacyjnej."
      );
    } else {
